...
export class NotFoundError extends Error {
  constructor({ cause, message, action }) {
    super(message || "Não foi possível encontrar este reurso no sistema", {
      cause,
    });
...
export class UnauthorizedError extends Error {
  constructor({ cause, message, action }) {
    ...
    this.action =
      action ||
      "Faça login novamente e verifique se os dados informados estão corretos.";
    this.statusCode = 401;
  }
...
export class ForbiddenError extends Error {
  constructor({ cause, message, action }) {
    ...
    this.action =
      action || "Verifique se você possui permissão para acessar este recurso.";
    this.statusCode = 403;
  }
